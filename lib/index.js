    ...
    visitor: {
      ...
      CallExpression: function CallExpression(path, state) {
        ...
        if (referencesImport(callee, FUNCTION_NAMES)) {
          var messagesObj = path.get('arguments')[1];
          var scope = getMessageDescriptorValue(path.get('arguments')[0]);
          assertObjectExpression(messagesObj);
          var properties = messagesObj.get('properties').map(function (prop) {
            var value = prop.get('value');
            var description = value.node.value;
            var id = "".concat(scope, ".").concat(prop.node.key.name);
            var parsed = t.objectProperty(t.stringLiteral(id), t.objectExpression([t.objectProperty(t.stringLiteral('id'), t.stringLiteral(id)), t.objectProperty(t.stringLiteral('defaultMessage'), t.stringLiteral(description))]));
            return parsed;
          });
